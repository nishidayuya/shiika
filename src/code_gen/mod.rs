...
pub struct CodeGen<'hir: 'ictx, 'run: 'ictx, 'ictx> {
...
fn run(hir: &Hir, filepath: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    } // ここでcode_genがdropされるのではないのか？
    Ok(())
}

impl<'hir: 'ictx, 'run: 'ictx, 'ictx> CodeGen<'hir, 'run, 'ictx> {
    pub fn new(hir: &'hir Hir,
               context: &'ictx inkwell::context::Context,
               module: &'ictx inkwell::module::Module<'ictx>,
               builder: &'ictx inkwell::builder::Builder<'ictx>)
